...
const Page_emprunt = () => {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				const response = await axios.get(`http://${API_IP}:3000/history`);

				// nom_objet, ID, date_prevu, date_reelle
				for (const user of response.data) {

                    // On récupère les items de l'historique de l'utilisateur
					const userHistory = user.history.items;

                    // Pour chaque item, on crée un nouvel objet User
					...
					// const item_keys = Object.keys(userHistory);
				}

				// response.data.forEach((entry) => {
				//     const userHistory = entry.history.items;
				//     const username = entry.username;
				//     const item_keys = Object.keys(userHistory);
				//     console.log(item_keys);
				// });

				// const formattedUsers = response.data.map((entry) => {
				// 	const user = entry.history.items;
				// 	return {
				// 		username: entry.username,
				// 		items: Object.entries(user).map(([nom_objet, details]) => ({
				// 			nom_objet,
				// 			ID: details._id,
				// 			date_prevu: details.plannedReturnDate || "N/A",
				// 			date_reelle: details.realReturnDate || "N/A",
				// 		})),
				// 	};
				// });
